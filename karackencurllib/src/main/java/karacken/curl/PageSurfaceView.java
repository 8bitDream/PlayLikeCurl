...
public class PageSurfaceView extends GLSurfaceView implements GestureDetector.OnGestureListener {
    ...
    private void processPage() {
        ...
        if (currentPosition == 0) {
            renderer.updatePageUris(pageCurlAdapter.getItemUri(currentPosition),
                    pageCurlAdapter.getItemUri(currentPosition),
                    pageCurlAdapter.getItemUri(currentPosition + 1));
            canSwipeLeft = false;
            canSwipeRight = true;
        } else if (currentPosition == pageCurlAdapter.getCount() - 1) {
            renderer.updatePageUris(pageCurlAdapter.getItemUri(currentPosition - 1),
                    pageCurlAdapter.getItemUri(currentPosition),
                    pageCurlAdapter.getItemUri(currentPosition));
            canSwipeLeft = true;
            canSwipeRight = false;
